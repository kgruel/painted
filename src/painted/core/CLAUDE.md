...
| **One wrap engine** | `block.py` (`--- Styled wrap engine ---`) | Wrapping operates on a *styled-char stream* (`_StyledChars`); the `str` paths are thin adapters over the same `_*_styled` cores. There is no parallel str/styled logic to keep in sync — don't add one. The `str` adapter is the degenerate single-style, no-ref case. |
