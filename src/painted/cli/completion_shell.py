...
def install_completion(
    shell: str, prog: str, *, aliases: Sequence[str] = (), dry_run: bool = False
) -> int:
    ...
    ``aliases`` are alternate program names the same glue should register for
    (e.g. loops installs ``loops`` and ``sl`` from one main) — the glue body is
    invocation-agnostic already, so this only widens registration.
...
def completion_handler(prog: str | None, aliases: Sequence[str] = ()) -> Callable[[list[str]], int]:
    """The ``completion`` command handler, closed over ``prog`` and ``aliases``.
    ...
    ``aliases`` are alternate program names (e.g. loops installs ``loops`` and
    ``sl`` from one main) — the glue registers for all of them, never
    auto-detected, only declared by the app that wires this handler up."""
    ...
    def handler(argv: list[str]) -> int:
        ...
        if ns.install or ns.dry_run:
            ...
            if shell is None:
                ...
                return 1
            return install_completion(shell, prog_name, aliases=aliases, dry_run=ns.dry_run)
        # print path: the benign zsh fallback is fine — it goes to stdout.
        shell = ns.shell or _detect_shell() or "zsh"
        sys.stdout.write(_EMITTERS[shell](prog_name, aliases))
        return 0

    return handler


def _zsh_script(prog: str, aliases: Sequence[str] = ()) -> str:
    ...
    callback works regardless of install path — including under any of
    ``aliases``, alternate entry points registered on the same ``#compdef``
    line (zsh's multi-name shorthand) so they get this glue instead of file
    completion.
...
def _bash_script(prog: str, aliases: Sequence[str] = ()) -> str:
    ...
    bash's ``complete`` has no ``#compdef``-style multi-name shorthand, so each
    of ``aliases`` gets its own ``complete -F`` line registering the same shared
    function — one function body, one registration per program name."""
    registrations = "\n".join(f"complete -F _{prog}_complete {name}" for name in (prog, *aliases))
    return f"""\
...
_EMITTERS: dict[str, Callable[[str, Sequence[str]], str]] = {
    "zsh": _zsh_script,
    "bash": _bash_script,
}
